...
rec {
  ...
  */
  rerunOnChange = { preOverrideAttrs ? attrs: attrs }@args:
    fixedOutputDerivation:
    assert (lib.assertMsg (fixedOutputDerivation ? outputHash)
      "rerunOnChange expects a fixedOutputDerivation");
    assert (lib.assertMsg (fixedOutputDerivation ? overrideAttrs)
    ...
    let
      ...
      inputsHash = let
        ...
        outputPath =
          builtins.unsafeDiscardStringContext unfixedDerivation.outPath;
      in builtins.substring 11 32 outputPath;

    in fixedOutputDerivation.overrideAttrs (attrs: {
      ...
      overrideAttrs = f:
        rerunOnChange args (fixedOutputDerivation.overrideAttrs f);
    };
}
