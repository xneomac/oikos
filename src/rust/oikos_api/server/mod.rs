...
/// Get info
async fn get_info<Server: OikosApi>(
    server: Data<Server>,
) -> impl Responder {
    use get_info::*;
    ...
    match server.get_info(parameters).await {
                            Ok(Success::Status200(response)) => HttpResponseBuilder::new(StatusCode::from_u16(200).unwrap()).json(response),
...
async fn get_recipes<Server: OikosApi>(
    ...
    match server.get_recipes(parameters).await {
                            Ok(Success::Status200(response)) => HttpResponseBuilder::new(StatusCode::from_u16(200).unwrap()).json(response),
                            Err(Error::Status401(response)) => HttpResponseBuilder::new(StatusCode::from_u16(401).unwrap()).json(response),
                            Err(Error::Status403(response)) => HttpResponseBuilder::new(StatusCode::from_u16(403).unwrap()).json(response),
        Err(Error::Unknown(err)) => HttpResponse::InternalServerError().body(err_to_string(&err)),
    }
}
/// Add recipe
...
async fn get_recipe_by_id<Server: OikosApi>(
    ...
    match server.get_recipe_by_id(parameters).await {
                            ...
                            Err(Error::Status401(response)) => HttpResponseBuilder::new(StatusCode::from_u16(401).unwrap()).json(response),
                            Err(Error::Status403(response)) => HttpResponseBuilder::new(StatusCode::from_u16(403).unwrap()).json(response),
                            Err(Error::Status404(response)) => HttpResponseBuilder::new(StatusCode::from_u16(404).unwrap()).json(response),
        Err(Error::Unknown(err)) => HttpResponse::InternalServerError().body(err_to_string(&err)),
    }
}
/// Update a recipe
...
pub fn config<Server: OikosApi + Send + Sync + Clone + 'static>(
    app: &mut ServiceConfig,
) {
    app
        .service(
            resource("/info")
                .route(get().to(get_info::<Server>))
        )
        .service(
            resource("/recipes")
